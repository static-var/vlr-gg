...
internal class VlrRepositoryTest {
  ...
  @Test
  fun `test if Ok(false) is returned when updateLatestMatches api call is successful`() = runTest(testDispatcher) {
  ...
  @Test
  fun `test if getMatchesFromDb gets data when api call is successful`() = runTest(testDispatcher) {
    TimeElapsed.reset(Endpoints.MATCHES_OVERVIEW) // Reset cache key for api calls
  ...
  @Test
  fun `test if getMatchesFromDb has same data as before when api call fails`() = runTest(testDispatcher) {
    TimeElapsed.reset(Endpoints.MATCHES_OVERVIEW) // Reset cache key for api calls
  ...
  @Test
  fun `test if Ok(false) is returned when updateLatestEvents api call is successful`() = runTest(testDispatcher) {
  ...
  @Test
  fun `test if Err(exception) is returned when updateLatestEvents api responds with error`() = runTest(testDispatcher) {
    TimeElapsed.reset(Endpoints.EVENTS_OVERVIEW) // Reset cache key for api calls
    vlrMockEngine.nextResponseWithServerError() // Ensure the api call fails
  ...
  @Test
  fun `test if getEventsFromDb gets data when api call is successful`() = runTest(testDispatcher) {
    TimeElapsed.reset(Endpoints.EVENTS_OVERVIEW) // Reset cache key for api calls
  ...
  @Test
  fun `test if getEventsFromDb has same data as before when api call fails`() = runTest(testDispatcher) {
    TimeElapsed.reset(Endpoints.EVENTS_OVERVIEW) // Reset cache key for api calls
  ...
  @Test
  fun `test if Ok(false) is returned when updateLatestMatchDetails api call is successful`() = runTest(testDispatcher) {
    val matchId = "107742"
  ...
  @Test
  fun `test if getMatchDetailsFromDb gets data when api call is successful`() = runTest(testDispatcher) {
    val matchId = "107742"
    TimeElapsed.reset(Endpoints.matchDetails(matchId)) // Reset cache key for api calls
  ...
  @Test
  fun `test if getMatchDetailsFromDb has same data as before when api call fails`() = runTest(testDispatcher) {
    val matchId = "107742"
    TimeElapsed.reset(Endpoints.matchDetails(matchId)) // Reset cache key for api calls
  ...
  @Test
  fun `test if Ok(false) is returned when updateLatestEventDetails api call is successful`() = runTest(testDispatcher) {
    val eventId = "800"
  ...
  @Test
  fun `test if getEventDetailsFromDb gets data when api call is successful`() = runTest(testDispatcher) {
    val eventId = "800"
    TimeElapsed.reset(Endpoints.eventDetails(eventId)) // Reset cache key for api calls
  ...
  @Test
  fun `test if getEventDetailsFromDb has same data as before when api call fails`() = runTest(testDispatcher) {
    val eventId = "800"
    TimeElapsed.reset(Endpoints.eventDetails(eventId)) // Reset cache key for api calls
...
//    }
  ...
  @Test
  fun `test if getTeamDetails gets data when api call is successful`() = runTest(testDispatcher) {
    val teamId = "2291"
  ...
  @Test
  fun `test if getTeamDetails fails gracefully`() = runTest(testDispatcher) {
    val teamId = "2291"
    vlrMockEngine.nextResponseWithServerError()
    repository.getTeamDetails(teamId).test {
      skipItems(1) // Waiting state emits first so skip that
